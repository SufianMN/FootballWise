...
import { Activity, BarChart2, GitCompare, Info, Trophy, List, User, Users, Sparkles } from 'lucide-react';
...
const MainLayout: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'AI Analyst', path: '/ai-analyst', icon: <Sparkles size={18} /> },
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      <header className="bg-surface border-b border-slate-700 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4 xl:gap-6">
          <Link to="/" className="flex items-center gap-2 px-3 py-2 rounded-md transition-all duration-200 text-textSecondary hover:text-white hover:bg-slate-800/40">
              ...
              <Link 
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-md transition-all duration-200 ${
                  location.pathname === link.path 
                    ? 'text-primary bg-slate-800/60 font-semibold shadow-sm' 
                    : 'text-textSecondary hover:text-white hover:bg-slate-800/40 font-medium'
                }`}
              >
                {link.icon}
                <span>{link.name}</span>
