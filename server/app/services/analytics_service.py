...
class AnalyticsService:
    ...
    def load_data(self):
        ...
        print("Analytics logs successfully cached.")
    ...
    def get_team_analytics(self, team_id: str):
        ...
        rolling_window = 5
        rolling_win_rate = team_df['win'].rolling(window=rolling_window, min_periods=1).mean()
        rolling_xg = team_df['xg'].rolling(window=rolling_window, min_periods=1).mean()
        rolling_goals = team_df['goals_scored'].rolling(window=rolling_window, min_periods=1).mean()
        rolling_poss = team_df['possession'].rolling(window=rolling_window, min_periods=1).mean()
        rolling_pass = team_df['pass_accuracy'].rolling(window=rolling_window, min_periods=1).mean()
        rolling_shots = team_df['shots'].rolling(window=rolling_window, min_periods=1).mean()
        
        for i, row in enumerate(team_df.itertuples()):
            date = row.match_date
            trends["form"].append({"match_date": date, "value": round(float(rolling_win_rate.iloc[i]) * 100, 1)})
            trends["xg"].append({"match_date": date, "value": round(float(rolling_xg.iloc[i]), 2)})
            trends["goals"].append({"match_date": date, "value": round(float(rolling_goals.iloc[i]), 2)})
            trends["possession"].append({"match_date": date, "value": round(float(rolling_poss.iloc[i]), 1)})
            trends["pass_accuracy"].append({"match_date": date, "value": round(float(rolling_pass.iloc[i]), 1)})
            trends["shots"].append({"match_date": date, "value": round(float(rolling_shots.iloc[i]), 1)})
