...
class PredictionService:
    ...
    def load_artifacts(self):
        ...
        models_dir = os.environ.get("MODEL_DIRECTORY", str(default_base_dir / "models"))
        ...
        if os.path.exists(pointer_path):
            try:
                ...
                logger.warning(f"Warning: Could not read latest_model_info.json: {e}")

        logger.info(
            f"Loading ML model artifacts from {models_dir} (version {version.strip('_')} if versioned)..."
        )
        ...
        try:
            ...
            if os.path.exists(fc_path):
                ...
                logger.warning(f"Feature columns not found at {fc_path}")
        except Exception as e:
            logger.error(f"Error loading prediction artifacts: {e}", exc_info=True)
    ...
    def predict(self, home_team_id, away_team_id):
        ...
        X, home_features, away_features, h2h_features = (
            feature_builder_service.build_match_features(
        ...
        if X is None:
            raise ValueError(
                "Could not find sufficient historical data for the selected teams"
            )
        ...
        res["match_preview"] = match_preview_service.generate_preview(
        ...
        top_features = explainability_service.generate_explanations(
            X, int(predicted_idx), self.feature_columns
        )
        insights = explainability_service.generate_insights(
            home_features, away_features, h2h_features
        )
