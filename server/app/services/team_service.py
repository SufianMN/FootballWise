...
class TeamService:
    ...
    def load_data(self):
        ...
        matches_path = os.path.join(data_dir, "processed", "processed_matches.csv")
        ...
        try:
            if os.path.exists(matches_path):
                ...
                home_teams = self.matches_df[
                    ["home_team_id", "home_team_name"]
                ].drop_duplicates()
                away_teams = self.matches_df[
                    ["away_team_id", "away_team_name"]
                ].drop_duplicates()

                for _, row in home_teams.iterrows():
                    if not pd.isna(row["home_team_id"]):
                        self.team_names[str(int(row["home_team_id"]))] = row[
                            "home_team_name"
                        ]
                for _, row in away_teams.iterrows():
                    if not pd.isna(row["away_team_id"]):
                        self.team_names[str(int(row["away_team_id"]))] = row[
                            "away_team_name"
                        ]
            else:
                logger.warning(f"Matches file not found at {matches_path}")
        except Exception as e:
            logger.error(f"Error loading matches_df in team_service: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Error loading dataset_df in team_service: {e}", exc_info=True)
    ...
    def get_team_stats(self, team_id: str):
        ...
        if not team_home.empty:
            latest_home = team_home.sort_values(by="match_date", ascending=False).iloc[
                0
            ]
        if not team_away.empty:
            latest_away = team_away.sort_values(by="match_date", ascending=False).iloc[
                0
            ]
        ...
        stats = {
            ...
            "goals": (
                round(latest_row[f"{prefix}avg_goals_scored_l5"], 2)
                if f"{prefix}avg_goals_scored_l5" in latest_row
                else 0
            ),
            "xg": (
                round(latest_row[f"{prefix}avg_xg_l5"], 2)
                if f"{prefix}avg_xg_l5" in latest_row
                else 0
            ),
            "possession": (
                round(latest_row[f"{prefix}possession_l5"], 2)
                if f"{prefix}possession_l5" in latest_row
                else 0
            ),
            "passing_accuracy": (
                round(latest_row[f"{prefix}pass_accuracy_l5"], 2)
                if f"{prefix}pass_accuracy_l5" in latest_row
                else 0
            ),
            "defensive_rating": (
                round(latest_row[f"{prefix}avg_goals_conceded_l5"], 2)
                if f"{prefix}avg_goals_conceded_l5" in latest_row
                else 0
            ),
            "win_rate": (
                round(latest_row[f"{prefix}win_rate_l5"], 2)
                if f"{prefix}win_rate_l5" in latest_row
                else 0
            ),
            "clean_sheets": (
                round(latest_row[f"{prefix}clean_sheets_l5"], 2)
                if f"{prefix}clean_sheets_l5" in latest_row
                else 0
            ),
            "shots": (
                round(latest_row[f"{prefix}shots_per_game_l5"], 2)
                if f"{prefix}shots_per_game_l5" in latest_row
                else 0
            ),
