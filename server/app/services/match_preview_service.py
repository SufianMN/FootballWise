...
class MatchPreviewService:
    def generate_preview(self, home_team_id: str, away_team_id: str):
        ...
        home_avg_goals = round(float(home_l5['goals_scored'].mean()), 2) if len(home_l5) > 0 else 0
        away_avg_goals = round(float(away_l5['goals_scored'].mean()), 2) if len(away_l5) > 0 else 0
        ...
        if team_service.matches_df is not None:
            ...
            h2h_matches = matches[
                ...
                ((matches['home_team_id'] == tid2) & (matches['away_team_id'] == tid1))
            ]
