...
class MatchService:
    ...
    def load_data(self):
        logger.info("Loading match explorer data...")
        ...
        if not os.path.exists(matches_path) or not os.path.exists(stats_path):
            logger.warning(f"Match data missing at {processed_dir}!")
            return
        ...
        try:
            ...
            logger.error(f"Error loading match data: {e}", exc_info=True)
            return
        ...
        logger.info("Match Explorer data successfully cached.")
    ...
    def search_matches(
        ...
        if team:
            df = df[
                (df["home_team_name"].str.contains(team, case=False, na=False))
                | (df["away_team_name"].str.contains(team, case=False, na=False))
            ]
        if date:
        ...
        return convert_numpy_types(
            {"total": total, "page": page, "page_size": page_size, "matches": matches}
        )
    ...
    def get_match_details(self, match_id: int):
            ...
            try:
                with open(events_file, "r", encoding="utf-8") as f:
                    ...
                    for ev in raw_events:
                        ...
                        if (
                            ev_type == "Shot"
                            and ev.get("shot", {}).get("outcome", {}).get("name")
                            == "Goal"
                        ):
                            desc = "Goal"
                            if (
                                ev.get("shot", {}).get("type", {}).get("name")
                                == "Penalty"
                            ):
                                desc = "Penalty Goal"
                            events.append(
                            ...
                            ev_type == "Shot"
                            and ev.get("shot", {}).get("outcome", {}).get("name")
                            in ["Saved", "Off T", "Post", "Wayward"]
                            and ev.get("shot", {}).get("type", {}).get("name")
                            == "Penalty"
                        ):
                            events.append(
                                {
                ...
                logger.error(f"Error parsing events for match {match_id}: {e}")
        ...
        home_poss = round(
            (home_passes / total_passes * 100) if total_passes > 0 else 50, 1
        )
        away_poss = round(
            (away_passes / total_passes * 100) if total_passes > 0 else 50, 1
        )
        ...
        if home_poss > 60:
            summary_sentences.append(
                f"{row['home_team_name']} dominated possession ({home_poss}%)."
            )
        elif away_poss > 60:
            summary_sentences.append(
                f"{row['away_team_name']} dominated possession ({away_poss}%)."
            )
        else:
            summary_sentences.append(
                f"Possession was evenly contested ({home_poss}% - {away_poss}%)."
            )

        if home_xg > away_xg and row["home_score"] <= row["away_score"]:
            summary_sentences.append(
                f"{row['home_team_name']} generated more expected goals ({home_xg} xG) but failed to secure the win."
            )
        elif away_xg > home_xg and row["away_score"] <= row["home_score"]:
            summary_sentences.append(
                f"{row['away_team_name']} generated more expected goals ({away_xg} xG) but failed to secure the win."
            )

        if (home_xg + away_xg) > 3.5:
            summary_sentences.append(
                "Both teams created numerous high-quality chances in an open game."
            )
        elif (home_xg + away_xg) < 1.5:
            summary_sentences.append(
                "It was a tight, defensive affair with few clear-cut opportunities."
            )

        if len(summary_sentences) == 1:
            summary_sentences.append(
                f"The match ended {int(row['home_score'])}-{int(row['away_score'])}."
            )
