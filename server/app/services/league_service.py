...
class LeagueService:
    ...
    def load_data(self):
        print("Loading league analytics data...")
        ...
        for _, row in df.iterrows():
            ...
            team_logs.append({
                ...
                'team_id': str(int(row['home_team_id'])) if pd.notnull(row['home_team_id']) else "unknown",
            ...
            team_logs.append({
                'competition_id': comp_id,
                'team_id': str(int(row['away_team_id'])) if pd.notnull(row['away_team_id']) else "unknown",
                ...
                'btts': 1 if (h_score > 0 and a_score > 0) else 0,
                'points': 3 if a_score > h_score else (1 if a_score == h_score else 0)
            })
            
        self.df_logs = pd.DataFrame(team_logs)
        self.df_logs = self.df_logs.fillna(0)
        print("League data successfully cached.")
    ...
    def get_league_analytics(self, competition_id: str):
        ...
        for (tid, tname), group in grouped:
            ...
            team_stats.append({
                ...
                "avg_shots_on_target": round(float(group['shots_on_target'].mean()), 1),
                ...
                "clean_sheet_pct": round(float(group['clean_sheet'].mean()) * 100, 1),
        ...
        df_table = df_stats.sort_values(['points', 'goal_difference', 'goals_for'], ascending=[False, False, False])
        ...
        # Rankings (Top 5)
        rankings = {
            "top_attack": df_stats.nlargest(5, 'goals_for')[['team_name', 'goals_for']].to_dict('records'),
            "top_defense": df_stats.nsmallest(5, 'goals_against')[['team_name', 'goals_against']].to_dict('records'),
            "top_possession": df_stats.nlargest(5, 'avg_possession')[['team_name', 'avg_possession']].to_dict('records'),
            "top_xg": df_stats.nlargest(5, 'avg_xg')[['team_name', 'avg_xg']].to_dict('records'),
            "top_pass_accuracy": df_stats.nlargest(5, 'avg_pass_accuracy')[['team_name', 'avg_pass_accuracy']].to_dict('records')
        }
        
        leaders = {
            "best_attack_team": df_stats.loc[df_stats['goals_for'].idxmax()]['team_name'],
            "best_attack_val": df_stats['goals_for'].max(),
            
            "best_defense_team": df_stats.loc[df_stats['goals_against'].idxmin()]['team_name'],
            ...
            "best_pass_team": df_stats.loc[df_stats['avg_pass_accuracy'].idxmax()]['team_name'],
