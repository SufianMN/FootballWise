...
class ExplainabilityService:
    ...
    def generate_explanations(self, feature_vector_df, predicted_class_idx, feature_names):
        ...
        for i, feature in enumerate(feature_names):
            ...
            display_name = display_name.replace("L5", "(Last 5)").replace("H2H", "Head-to-Head")
    ...
    def generate_insights(self, home_features, away_features, h2h_features):
        ...
        if hw_streak >= 3:
            ...
            insights.append(f"Away team enters on a strong {int(aw_streak)}-match winning streak.")
            
        hu_streak = home_features.get('home_unbeaten_streak', 0)
        if hu_streak >= 5 and hw_streak < 3:
            insights.append(f"Home team is solid, carrying a {int(hu_streak)}-match unbeaten streak.")
        ...
        if home_xg > away_xg + 0.6:
            insights.append(f"Home side generates significantly higher attacking threat (xG: {home_xg:.2f} vs {away_xg:.2f}).")
        elif away_xg > home_xg + 0.6:
            insights.append(f"Away team has a noticeably superior expected goals average ({away_xg:.2f} per game).")
        ...
        if away_xga >= 2.0:
            insights.append(f"Away team has been defensively vulnerable, conceding {away_xga:.1f} goals per game recently.")
        if home_xga >= 2.0:
            insights.append(f"Home team defense is struggling, allowing {home_xga:.1f} goals per match.")
        ...
        if h2h_hw > h2h_aw + 1 and h2h_hw >= 2:
            insights.append("The home team has historically dominated recent head-to-head meetings.")
        elif h2h_aw > h2h_hw + 1 and h2h_aw >= 2:
            insights.append("The away team holds a strong psychological edge in recent head-to-head fixtures.")

        # 5. Clean Sheets
        home_cs = home_features.get('home_clean_sheets_l5', 0)
        if home_cs >= 3:
            insights.append(f"Home team boasts a sturdy defense with {int(home_cs)} clean sheets in their last 5.")

        # Fallback if too few insights
        if len(insights) == 0:
            insights.append("Both teams are relatively evenly matched in recent statistical metrics.")
            insights.append("This fixture could be highly contested based on current form.")
