...
class PlayerService:
    ...
    def load_data(self):
        ...
        # Calculate robust absolute path
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../"))
        csv_path = os.path.join(base_dir, "ml", "data", "processed", "player_stats.csv")
        
        if not os.path.exists(csv_path):
            print(
                "============================================================\n"
                "ERROR: Player statistics dataset not found.\n"
                "The server will start, but Player Analytics will be empty.\n"
                "To fix this, run the following command before starting the server:\n\n"
                "    python ml/build_player_dataset.py\n"
                "============================================================"
            )
            self.players_df = pd.DataFrame()
            return
        ...
        try:
            ...
            print(f"Loaded {len(self.players_df)} player season records.")
    ...
    def get_players(self):
        ...
        try:
            ...
            unique_players = sorted_df.drop_duplicates(subset=['player_id'], keep='first')
            
            result = unique_players[['player_id', 'player_name', 'team_name', 'position']].to_dict('records')
    ...
    def get_player_details(self, player_id: int):
        ...
        valid_players = all_matches[all_matches >= 5].index # Filter players with at least 5 matches
        ...
        agg_df['defending_pm'] = (agg_df['tackles'] + agg_df['interceptions']) / agg_df['matches']
        agg_df['progression_pm'] = agg_df['key_passes'] / agg_df['matches']
        
        def get_percentile(column, value):
            if agg_df.empty or matches == 0: return 50
            return int((agg_df[column] <= value).mean() * 100)

        radar = {
            'Goals': get_percentile('goals_pm', goals_per_match),
            'xG': get_percentile('xg_pm', xg_per_match),
            'Assists': get_percentile('assists_pm', (assists/matches) if matches>0 else 0),
            'Passing': get_percentile('passes_pm', (passes/matches) if matches>0 else 0),
            'Dribbling': get_percentile('dribbles_pm', (successful_dribbles/matches) if matches>0 else 0),
            'Defending': get_percentile('defending_pm', ((tackles+interceptions)/matches) if matches>0 else 0),
            'Progression': get_percentile('progression_pm', (key_passes/matches) if matches>0 else 0),
        }
